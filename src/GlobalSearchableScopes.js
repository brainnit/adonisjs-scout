...
const debug = require('debug')('scout:globalscopes')
...
class ScopeIterator {
  ...
  ignore (scopes) {
    if (scopes && !Array.isArray(scopes)) {
      throw CE.InvalidArgumentException.invalidParameter('ignoreScopes expects an array of names to ignore', scopes)
    }
   ...
   *
   * @param {Builder} builder Query builder reference
   *
   * @return void
   */
  execute (builder) {
...
class GlobalSearchableScopes {
  ...
  add (callback, name = null) {
    ...
    if (typeof (callback) !== 'function') {
      throw CE.InvalidArgumentException.invalidParameter('Model.addGlobalSearchableScope expects a closure as first parameter', callback)
    }
    ...
    if (name && typeof (name) !== 'string') {
      throw CE.InvalidArgumentException.invalidParameter('Model.addGlobalSearchableScope expects a string for the scope name', name)
    }
    ...
    if (name && this.scopes.find((scope) => scope.name && scope.name === name)) {
      throw CE.RuntimeException.invoke(`A scope with name ${name} alredy exists. Give your scope a unique name`)
    }
...
module.exports = GlobalSearchableScopes
